...
public class CE80D {
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for(int x=0; x<m; x++) {
			int id = sc.nextInt();
			int treeid = remap.get(id);
			
			int rank = tree.rank(treeid);
			
			if(rank+1 > maxs.get(id)) {
				maxs.put(id, rank+1);
			}
			mins.put(id, 1);
		...
		Iterable<Entry<Integer, Integer>> order = tree.levelOrder();
		for(Entry<Integer, Integer> e : order){
			int rank = e.getValue() + 1;
			if(rank > maxs.get(e.getKey())) {
				maxs.put(e.getKey(), rank);
			}
		}
		...
		for (int id=1; id<=n; id++) {
			...
			System.out.println(mins.get(id)+" "+maxs.get(id));
		}
	}
...
class BST<Key extends Comparable<Key>, Value> {
     ...
     */
    public Iterable<Entry<Key,Integer>> levelOrder() {
        Queue<Entry<Key,Integer>> keys = new LinkedList<Entry<Key,Integer>>();
        Queue<Entry<Node, Integer>> queue = new LinkedList<Entry<Node, Integer>>();
        queue.add(new AbstractMap.SimpleEntry<Node,Integer>(root, 0));
        while (!queue.isEmpty()) {
        	Entry<Node, Integer> e = queue.remove();
        	Node x = e.getKey();
        	int rnk = e.getValue();
            if (x == null) continue;
            keys.add(new AbstractMap.SimpleEntry<Key,Integer>(x.key, rnk+x.left.size));
            queue.add(new AbstractMap.SimpleEntry<Node,Integer>(x.left, rnk));
            queue.add(new AbstractMap.SimpleEntry<Node,Integer>(x.right, rnk + x.left.size + 1));
        }
        return keys;
    }
